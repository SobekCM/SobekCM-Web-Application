...
namespace SobekCM.Library.ItemViewer.Viewers
{
    ...
    public class Video_ItemViewer_Prototyper : iItemViewerPrototyper
    {
        ...
        {
            bool IpRestricted = RequestSpecificValues.Flags.ItemRestrictedFromUser;
        ...
        public virtual iItemViewer Create_Viewer(BriefItemInfo CurrentItem, RequestCache RequestSpecificValues, Custom_Tracer Tracer)
        {
            ...
            return new Video_ItemViewer(CurrentItem, CurrentUser, CurrentRequest, Tracer, FileExtensions, Context);
        }
    }
    ...
    public class Video_ItemViewer : abstractNoPaginationItemViewer
    {
        ...
        public Video_ItemViewer(BriefItemInfo BriefItem, User_Object CurrentUser, Navigation_Object CurrentRequest, Custom_Tracer Tracer, string[] FileExtensions, HttpContext context)
        {
            ...
            video = 1;
            string videoParam = context?.Request.Query["video"].ToString();
            if (!String.IsNullOrEmpty(videoParam))
            {
                int tryVideo;
                if (Int32.TryParse(videoParam, out tryVideo))
                {
                    if (tryVideo < 1)
                        tryVideo = 1;
                    video = tryVideo;
