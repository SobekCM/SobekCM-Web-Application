...
namespace SobekCM.Library.MySobekViewer
{
    ...
    public class New_TEI_MySobekViewer : abstract_MySobekViewer
    {
        ...
        public New_TEI_MySobekViewer(RequestCache RequestSpecificValues, HttpContext Context) : base(RequestSpecificValues, Context)
        {
            ...
            if (Context.SessionObject()["TEI_Item"] == null)
            {
            ...
            if ((RequestSpecificValues.Current_Mode.isPostBack) && (Context.Request.HasFormContentType))
            {
                ...
                if (action == "cancel")
                {
                    ...
                    Context.SessionObject()["New_TEI_mySobekViewer.Mapping_File"] = null;
                ...
                if (action == "next_phase")
                {
                    ...
                    {
                        // Save to the item
                        completeTemplate.Save_To_Bib(item, RequestSpecificValues.Current_User, currentProcessStep - 4, Context);
                        item.Save_METS();
                        Context.SessionObject()["Item"] = item;
