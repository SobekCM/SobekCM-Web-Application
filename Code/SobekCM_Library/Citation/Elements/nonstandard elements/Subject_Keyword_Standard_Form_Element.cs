        ...
        public override void Render_Template_HTML(TextWriter Output, SobekCM_Item Bib, string Skin_Code, bool IsMozilla, StringBuilder PopupFormBuilder, User_Object Current_User, string CurrentLanguage, Language_Support_Info Translator, string Base_URL)
        {
            ...
            foreach (Subject_Info thisSubject in Bib.Bib_Info.Subjects)
            {
                ...
                if (thisSubject.Class_Type == Subject_Info_Type.Standard)
                {
                    ...
                    {
                        PopupFormBuilder.Append("<input class=\"formsubject_medium_input sbk_Focusable\" name=\"formsubjecttopic1_" + subject_index + "\" id=\"formsubjecttopic1_" + subject_index + "\" type=\"text\" value=\"" + System.Net.WebUtility.HtmlEncode(standSubject.Topics[0]) + "\" />");
                    }
                    else
                    {
                    ...
                    PopupFormBuilder.AppendLine("      <td colspan=\"3\"><button title=\"Close\" class=\"sbkMetadata_RoundButton\" onclick=\"return close_subject_form('form_subject_" + subject_index + "');\">" + Localization_Gateway.Buttons.Close(CurrentLanguage) + "</button></td>");
