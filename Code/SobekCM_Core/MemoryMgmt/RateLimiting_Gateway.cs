...
{
    /// <summary> Per-IP request rate limiting: an IP making more than <see cref="RequestLimit"/> requests
    /// within <see cref="WindowSeconds"/> is banned for <see cref="BanMinutes"/>. Counter and ban state
    /// live in <see cref="SharedCache"/> under their own key prefixes (a fourth caller pattern alongside
    /// the three documented on SharedCache itself), so they need no separate store and are wiped along
    /// with everything else by Clear_Cache()/RemoveAll(). </summary>
    /// <remarks> Config is set once from Program.cs (same pattern as <c>ExceptionLog_Gateway</c>), since
    /// SobekCM_Core has no direct access to IConfiguration. Being SharedCache-backed, this is in-process
    /// and per-instance -- fine for a single-server deployment, but a load-balanced one would need a
    /// shared store for a ban to hold across instances. </remarks>
    public static class RateLimiting_Gateway
    {
        ...
        /// <summary> Records one request from the given IP against its current window, banning the IP
        /// if this push puts it over the limit. </summary>
        /// <returns> NULL if the request is allowed; otherwise how many minutes remain on the IP's ban </returns>
        public static int? CheckAndRecord(string ipAddress)
        {
            if ((!Enabled) || (string.IsNullOrEmpty(ipAddress)))
                return null;

            string banKey = BanKeyPrefix + ipAddress;
            if (SharedCache.Instance[banKey] is DateTime bannedUntil)
            {
            ...
            SharedCache.Instance.Set(banKey, banUntil, new MemoryCacheEntryOptions
            {
