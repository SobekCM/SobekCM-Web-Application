...
{
    /// <summary> Element allows simple entry of the course note(s) for an item </summary>
    /// <remarks> This class extends the <see cref="TextArea_Element"/> class. </remarks>
    public class Course_Notes_Element : TextArea_Element
    {
        /// <summary> Constructor for a new instance of the Course_Notes_Element class </summary>
        public Course_Notes_Element()
            : base("Course Notes", "course_note")
        {
            Repeatable = false;
        }
        ...
        public override void Render_Template_HTML(TextWriter Output, SobekCM_Item Bib, string Skin_Code, bool IsMozilla, StringBuilder PopupFormBuilder, User_Object Current_User, Web_Language_Enum CurrentLanguage, Language_Support_Info Translator, string Base_URL)
        {
            ...
            if (Acronym.Length == 0)
            {
                const string defaultAcronym = "Enter any notes regarding the course related to this material";
        ...
        /// <param name="Current_User"> Current user, who's rights may impact the way an element is rendered </param>
        /// <remarks> This clears any preexisting notes which are course notes </remarks>
        public override void Prepare_For_Save(SobekCM_Item Bib, User_Object Current_User)
        {
            if (Bib.Bib_Info.Notes_Count > 0)
            {
                List<Note_Info> deleteNotes = Bib.Bib_Info.Notes.Where(thisNote => (thisNote.Note_Type == Note_Type_Enum.Course)).ToList();
