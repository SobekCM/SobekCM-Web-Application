...
namespace SobekCM.Library.Helpers.UploadiFive
{
	...
	public class UploadiFive
	{
		...
		public void Add_To_Stream(TextWriter Output, HttpContext Context)
		{
			// If there is no current HTTPContext, can't do this...
			if ((UploadPath.Length > 0) && (Context != null))
			{
				...
				Context.SessionObject()["#UPLOADIFIVE::" + newToken.ThisGuid.ToString()] = newToken;
			}
		...
		public string HTML_To_Write(HttpContext Context)
		{
			StringBuilder builder = new StringBuilder(500);
			TextWriter writer = new StringWriter(builder);
			Add_To_Stream(writer, Context);
			writer.Close();
