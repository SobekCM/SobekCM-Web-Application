...
namespace SobekCM.Library.AdminViewer
{
    ...
    /// </ul></remarks>
    public class OpenNJ_Users_AdminViewer : abstract_AdminViewer
    {
        ...
        private const bool is_open_nj = true;
        ...
        public OpenNJ_Users_AdminViewer(RequestCache RequestSpecificValues)
            : base(RequestSpecificValues)
        {
            ...
            if (!String.IsNullOrEmpty(RequestSpecificValues.Current_Mode.My_Sobek_SubMode))
            {
                try
                {
                    int edit_userid = Convert.ToInt32(RequestSpecificValues.Current_Mode.My_Sobek_SubMode.Replace("a", "").Replace("b", "").Replace("c", "").Replace("d", "").Replace("e", "").Replace("v", ""));
            ...
            if (RequestSpecificValues.Current_Mode.isPostBack)
            {
                ...
                if ((mode == Users_Admin_Mode_Enum.Edit_User) && (editUser != null))
                {
                    ...
                    // Allow page 5 if this is open-nj or another otc
                    if (( is_open_nj) && ((RequestSpecificValues.Current_Mode.My_Sobek_SubMode.IndexOf("e") > 0)))
                        page = 5;
        ...
        private void Write_View_User_Form(TextWriter Output)
        {
            ...
            if (editUser.Is_User_Admin)
                text_builder.Append("Is user adminitrator<br />");
        ...
        private void Write_Edit_User_Form(TextWriter Output, Custom_Tracer Tracer)
        {
            ...
            // Only allow the special instructor tab if this is open-nj
            if (is_open_nj)
            {
                if (RequestSpecificValues.Current_Mode.My_Sobek_SubMode.IndexOf("e") > 0) page = 5;
            }
            ...
            if (is_open_nj)
            {
                if (page == 5)
                {
                    Output.WriteLine("      <li class=\"tabActiveHeader\"> INSTRUCTOR </li>");
                }
                else
                {
                    Output.WriteLine("    <li onclick=\"return new_user_edit_page('" + editUser.UserID + "e" + "');\">" + " INSTRUCTOR " + "</li>");
                }
            }
