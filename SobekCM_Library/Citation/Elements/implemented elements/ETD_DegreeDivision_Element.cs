...
namespace SobekCM.Library.Citation.Elements
{
	...
	public class ETD_DegreeDivision_Element : MultipleTextBox_Element
	{
		...
		public override void Render_Template_HTML(TextWriter Output, SobekCM_Item Bib, string Skin_Code, bool IsMozilla, StringBuilder PopupFormBuilder, User_Object Current_User, Web_Language_Enum CurrentLanguage, Language_Support_Info Translator, string Base_URL)
		{
			...
			if (Acronym.Length == 0)
			{
				Acronym = "Enter the division(s) for this thesis/dissertation";
			}
			...
			if ((etdInfo == null) || (etdInfo.Degree_Divisions_Count == 0))
			{
				render_helper(Output, String.Empty, Skin_Code, Current_User, CurrentLanguage, Translator, Base_URL);
			}
			else
			{
				if (etdInfo.Degree_Divisions_Count == 1)
				{
					render_helper(Output, etdInfo.Degree_Divisions[0], Skin_Code, Current_User, CurrentLanguage, Translator, Base_URL);
				}
				else
				{
					render_helper(Output, etdInfo.Degree_Divisions, Skin_Code, Current_User, CurrentLanguage, Translator, Base_URL);
				}
			}
		}
		...
		public override void Prepare_For_Save(SobekCM_Item Bib, User_Object Current_User)
		{
			Thesis_Dissertation_Info etdInfo = Bib.Get_Metadata_Module(GlobalVar.THESIS_METADATA_MODULE_KEY) as Thesis_Dissertation_Info;
			if (etdInfo != null)
				etdInfo.Clear_Degree_Divisions();
		}
			...
			foreach (string thisKey in getKeys)
			{
				if (thisKey.IndexOf(html_element_name.Replace("_", "")) == 0)
				{
					...
					if (value.Length > 0)
					{
						...
						etdInfo.Add_Degree_Division(value);
					}
				}
			}
		}
