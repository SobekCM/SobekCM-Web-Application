...
{
	/// <summary> Element allows entry of the ETD degree metadata for an item </summary>
	/// <remarks> This class extends the <see cref="SimpleTextBox_Element"/> class. </remarks>
	public class ETD_Degree_Element : SimpleTextBox_Element
	{
		/// <summary> Constructor for a new instance of the ETD_Degree_Element class </summary>
		public ETD_Degree_Element()
			...
			foreach (string thisKey in getKeys)
			{
				if (thisKey.IndexOf(html_element_name.Replace("_", "")) == 0)
				{
					...
					if (value.Length > 0)
					{
						...
						etdInfo.Degree = value;
					}
					else
					{
						if (etdInfo != null)
							etdInfo.Degree = String.Empty;
					}
					return;
				}
			}
